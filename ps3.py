...
def parse_event(event):
  ...
  if debug:
    print("action {0} time: {1}, val: {2}, type: {3}, num: {4}"
          .format(action, val, action_type, action_num))
...
def step_action(action, action_value, state, state_value):
  ...
  if action == "forward" and state == "stopped":
    ...
    state = "stopped"  # stoppingを追加する
  elif action == "rotation" and state == "forward":
    state = "forward"
    state_value = convert_theta(state_value)
  elif debug and action:
    print("UNKNOWN ACTION: {0}".format(action))
  return (state, state_value)

def direction2speed(direction_value, default_speed = 5000):
